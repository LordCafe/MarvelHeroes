...
const createMapKey = function (index, unikey) {
    let key = "map-creator" +"-"+ index ;
    if( unikey ) key = key +"-"+ Math.random();
    return key;
}

const ComponetMaker = function (Template, stack, connection = function () { }, mark = false, randomKey = false) {
    let newStack = stack.map((data, index) => (< Template
        key={createMapKey(index, randomKey )}
        {...data}
        connect={connection}
        index={index}
        mark={mark}
